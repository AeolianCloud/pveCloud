...
const userEditorRules: FormRules = {
  username: [
  ...
  password: [
    {
      validator: (_rule: any, value: string) => {
        if (!isUserCreateMode.value && !value) return true
        if (!value) return new Error('请输入登录密码')
...
const roleEditorRules: FormRules = {
  code: [
...
async function handleUserRefresh() {
  ...
  try {
    ...
    await Promise.all(tasks)
    message.success('管理员数据已刷新')
  } catch (error) {
    message.error(toErrorMessage(error, '刷新失败'))
  } finally {
...
async function searchUsers() {
  ...
  try {
    ...
    message.error(toErrorMessage(error, '管理员列表加载失败'))
  } finally {
...
async function searchRoles() {
  ...
  try {
    ...
    message.error(toErrorMessage(error, '管理组列表加载失败'))
  } finally {
...
async function searchSessions() {
  ...
  try {
    ...
    message.error(toErrorMessage(error, '管理员会话加载失败'))
  } finally {
...
async function submitUserEditor() {
  ...
  try {
    ...
    if (payload.role_ids && findInactiveRoleIds(payload.role_ids).length > 0) {
      message.warning('已停用角色不能继续分配，请先移除')
      return
    }
...
async function submitPasswordReset() {
  ...
  try {
    ...
    message.error(toErrorMessage(error, '密码重置失败'))
  } finally {
...
async function toggleUserStatus(user: AdminUserItem) {
  ...
  try {
    await confirm({
      title: '确认状态切换',
      content: `确认将管理员“${user.display_name}”设为${nextLabel}吗？`,
  ...
  try {
    ...
    message.success(`管理员状态已更新为${nextLabel}`)
    await searchUsers()
  } catch (error) {
    message.error(toErrorMessage(error, '状态更新失败'))
  } finally {
...
async function toggleRoleStatus(role: AdminRoleItem) {
  ...
  try {
    await confirm({
      title: '确认状态切换',
      content: `确认将管理组“${role.name}”设为${nextLabel}吗？`,
      type: nextStatus === 'disabled' ? 'warning' : 'info',
    })
  } catch {
    return
  ...
  try {
    ...
    message.success(`管理组状态已更新为${nextLabel}`)
    await reloadRoleDataForAllViews()
  } catch (error) {
    message.error(toErrorMessage(error, '状态更新失败'))
  } finally {
...
async function revokeSession(session: AdminSessionItem) {
  ...
  try {
    await confirm({
      title: '确认吊销会话',
      content: `确认吊销管理员“${targetLabel}”的会话吗？`,
      type: 'warning',
      positiveText: '确认吊销',
    })
  } catch {
  ...
  try {
    ...
    message.success('管理员会话已吊销')
    await searchSessions()
  } catch (error) {
    message.error(toErrorMessage(error, '吊销会话失败'))
  } finally {
  ...
  <div class="admin-settings-page">
    ...
    <QueryState :loading="initialLoading" :error-message="errorMessage" @retry="initializePage">
      <n-tabs v-model:value="activeTab" type="line">
        <n-tab-pane v-if="canViewUsersTab" name="users" tab="管理员账号">
          <AdminUsersTab
        ...
        <n-tab-pane v-if="canViewRolesTab" name="roles" tab="管理组权限">
          <AdminRolesTab
        ...
        <n-tab-pane v-if="canViewSessionsTab" name="sessions" tab="管理员会话">
          <AdminSessionsTab
