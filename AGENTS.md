...
- 接口文档和接口契约写在 `docs/server/api/` 和对应业务文档中，不再维护 OpenAPI 生成文件。
...
- 新增或修改接口时，先更新 `docs/server/api/` 和对应业务文档；不要只改技能 reference。
...
- `docs/server/api/` 和对应业务文档是接口最终契约来源；数据库迁移 SQL 是表结构最终契约。
