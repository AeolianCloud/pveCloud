...
- 生成或修改代码后，最终回复需要基于当前实现给出类似功能、后续优化或可改进方向，供维护者判断是否继续推进；如果本次只是纯文档、纯排查或无明显后续方向，可以简短说明暂无建议。

## 代码书写规则

- 函数、类型、接口和包级变量注释采用块注释标签风格，使用 `/** ... */`，第一段用中文说明用途；必要时补充 `@param`、`@return`、`@error`、`@sideeffect` 等标签。
- API handler 注释可以使用 `@route`、`@request`、`@response`、`@auth` 等标签描述接口行为；这些标签必须与 `docs/server/api/openapi.yaml` 保持一致，OpenAPI 3.x 规范文件仍是最终接口契约来源。
- 代码内部注释使用详细中文，重点解释不直观逻辑、业务规则、幂等、事务、权限、补偿、并发和外部系统边界。
- 不写重复代码含义的空注释，例如“设置变量”“调用函数”这类注释不要写。
- 日志、错误信息、API `message`、命令行提示和面向后台/用户/运维的返回信息统一使用中文。
- 第三方协议要求固定英文响应时，可以按协议返回英文，但必须在代码附近用中文注释说明原因。
