...
func AdminAuth(cfg bootstrap.JWTConfig, db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		roleIDs, err := support.RoleIDs(c.Request.Context(), db, admin.ID)
		if err != nil {
			response.Error(c, err)
			c.Abort()
			return
		}
		permissionCodes, err := support.PermissionCodes(c.Request.Context(), db, admin.ID)
		if err != nil {
			response.Error(c, err)
			c.Abort()
			return
		...
		auditCtx := requestcontext.WithRequestContext(c.Request.Context(), requestcontext.RequestContext{
			AdminID:          &admin.ID,
