...
func (s *RealNameService) Submit(ctx context.Context, userID uint64, req webdto.RealNameSubmitRequest) (webdto.RealNameSubmitResponse, error) {
	...
	var created mysqlrealname.UserRealNameApplication
	...
	if err := mysqltx.NewManager(s.db).WithinContext(ctx, func(tx *gorm.DB) error {
		...
		created = mysqlrealname.UserRealNameApplication{
...
func (s *RealNameService) syncApplicationByID(ctx context.Context, id uint64, hook SyncApplicationHook, allowUnavailable bool) (mysqlrealname.UserRealNameApplication, error) {
...
func (s *RealNameService) markProviderCreationFailed(ctx context.Context, id uint64, providerErr error) error {
	...
	_ = providerErr
	return s.applications.UpdateApplication(ctx, nil, id, map[string]any{
...
func (s *RealNameService) userApplicationForSync(ctx context.Context, userID uint64, applicationNo string) (mysqlrealname.UserRealNameApplication, error) {
	app, err := s.applications.UserApplicationForSync(ctx, userID, applicationNo, statusPending)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return mysqlrealname.UserRealNameApplication{}, apperrors.ErrNotFound.WithMessage("实名申请不存在")
		}
	...
	if app.Status != statusPending {
		return mysqlrealname.UserRealNameApplication{}, apperrors.ErrConflict.WithMessage("当前实名申请无需同步")
	}
	return app, nil
}

func (s *RealNameService) applicationByProviderSession(ctx context.Context, provider string, providerApplicationID string) (mysqlrealname.UserRealNameApplication, error) {
	app, err := s.applications.ApplicationByProviderSession(ctx, provider, providerApplicationID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return mysqlrealname.UserRealNameApplication{}, apperrors.ErrNotFound.WithMessage("实名申请不存在")
	}
	return app, err
...
func (s *RealNameService) allowCallbackReplay(ctx context.Context, provider string, replayKey string, payloadDigest string) (bool, error) {
	if s.redis == nil || strings.TrimSpace(replayKey) == "" {
		return domainrealname.AllowCallbackReplay(replayKey, true), nil
	}
	...
	return domainrealname.AllowCallbackReplay(replayKey, ok), nil
}
...
func (s *RealNameService) latest(ctx context.Context, db *gorm.DB, userID uint64) (mysqlrealname.UserRealNameApplication, bool, error) {
	app, err := s.applications.LatestApplication(ctx, db, userID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return mysqlrealname.UserRealNameApplication{}, false, nil
	}
	return app, err == nil, err
...
func (s *RealNameService) latestForUpdate(ctx context.Context, db *gorm.DB, userID uint64) (mysqlrealname.UserRealNameApplication, bool, error) {
	app, err := s.applications.LatestApplicationForUpdate(ctx, db, userID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return mysqlrealname.UserRealNameApplication{}, false, nil
	}
	return app, err == nil, err
...
func applicationSummary(app mysqlrealname.UserRealNameApplication) webdto.RealNameApplicationSummary {
