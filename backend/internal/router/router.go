...
func New(db *gorm.DB, log *zap.Logger, cfg *config.Config) *gin.Engine {
	...
	// 404 / 405 自定义 JSON 响应，避免返回 HTML
	r.NoRoute(func(c *gin.Context) {
		c.JSON(404, gin.H{"code": 10002, "message": "接口不存在"})
	})
	r.NoMethod(func(c *gin.Context) {
		c.JSON(405, gin.H{"code": 10001, "message": "请求方法不允许"})
	})
	...
	// ── 初始化 service ────────────────────────────────────
	authSvc       := svcauth.New(db, cfg.JWT.Secret, cfg.JWT.ExpireHours)
	...
	// ── 初始化 handler ────────────────────────────────────
	authHandler       := handlerauth.New(authSvc)
