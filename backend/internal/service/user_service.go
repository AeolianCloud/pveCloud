...
var (
	...
	errAccountDisabled          = errors.New("账号已禁用，请联系管理员")
	errRefreshTokenRevoked      = errors.New("refresh token 已失效，请重新登录")
	errEmailVerifyTokenInvalid  = errors.New("邮箱验证链接无效或已过期")
)
...
func NewUserService(
	...
	return &UserService{userRepo: userRepo, jwt: jwt, tokenStore: tokenStore, emailSender: emailSender}
}
...
func (s *UserService) Register(ctx context.Context, email, password string) error {
	...
	if s.tokenStore != nil && s.tokenStore.Enabled() {
		if err := s.tokenStore.SaveEmailVerificationToken(ctx, verifyToken, user.ID, 24*time.Hour); err != nil {
			return err
		}
	}
	return s.emailSender.SendVerificationEmail(ctx, email, verifyToken)
}
...
// VerifyEmail 消费验证 token 后将用户标记为邮箱已验证。
func (s *UserService) VerifyEmail(ctx context.Context, verifyToken string) error {
	if s.tokenStore == nil || !s.tokenStore.Enabled() {
		return errors.New("邮箱验证服务未启用，请联系管理员")
	}
	userID, err := s.tokenStore.ConsumeEmailVerificationToken(ctx, verifyToken)
	if err != nil {
