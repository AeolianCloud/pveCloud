...
| action | object_type | 说明 |
| --- | --- | --- |
| `invoice.accept` | `invoice` | 管理端受理发票申请 |
| `invoice.reject` | `invoice` | 管理端驳回发票申请 |
| `invoice.issue` | `invoice` | 管理端登记开票并绑定 PDF |
| `invoice.admin_note.update` | `invoice` | 管理端更新发票后台备注 |

### 工单

工单关联实例只在用户端创建工单时保存业务实例编号，用于后台排障定位；不新增管理端写接口和后台审计动作。
...
- 发票：受理、驳回、开票登记和后台备注更新必须写审计；用户端创建和取消申请不写后台操作审计，但必须保存业务状态和请求链路标识。审计不得保存 PDF 内容、完整税号或物理文件路径。
