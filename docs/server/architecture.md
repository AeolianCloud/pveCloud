...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API，并开放最小 Web 公开站点配置、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口和订单 MVP。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单 MVP 接口和健康检查

当前契约重新开放用户账号自助、用户实名、服务器产品目录和订单 MVP；支付、实例、Worker、工单和其他用户端业务流仍不纳入现阶段交付范围。这里的支付宝/微信仅用于实名核验，不代表支付能力已开放。
...
当前仓库除站点配置、用户账号自助、支付宝/微信侧用户实名、服务器产品目录和订单 MVP 外，不再把其它 `/api/*` 作为现行后端契约。
...
当前后端领域边界以管理端基础能力为主，同时已重新开放用户账号自助、支付宝/微信侧用户实名、服务器产品目录和订单 MVP：
...
用户账号自助、支付宝/微信侧用户实名、服务器产品目录和订单 MVP 已经重新纳入当前阶段契约。支付、实例、工单、异步任务和其它用户端业务流仍从当前阶段契约中收口。后续如需恢复，必须先更新文档与迁移。
...
当前重新开放服务器产品目录、用户实名和订单相关数据库契约；实名供应商集成只覆盖支付宝/微信侧实名核验，不开放支付交易。支付、实例、工单、异步任务等业务域仍不属于当前阶段。
