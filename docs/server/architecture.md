...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API，并开放最小 Web 公开站点配置、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单、实例和工单能力。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单接口、实例接口、工单接口和健康检查

当前契约重新开放用户账号自助、用户实名、服务器产品目录、订单、实例和工单；支付、Worker、钱包和其他用户端业务流仍不纳入现阶段交付范围。这里的支付宝/微信仅用于实名核验，不代表支付能力已开放。实例交付通过后端内部 MCP PVE client API 调用上游 PVE 适配服务，不向前端暴露 PVE 节点、存储、磁盘来源或 VMID。
...
当前仓库除站点配置、用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、实例和工单外，不再把其它 `/api/*` 作为现行后端契约。
...
当前后端领域边界以管理端基础能力为主，同时已重新开放用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、实例和工单：
...
用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、实例和工单已经重新纳入当前阶段契约。支付、钱包和其它用户端业务流仍从当前阶段契约中收口。实例只覆盖 MCP PVE client API 当前具备的创建、查询、启动、停止、删除和异步操作查询能力。
...
当前重新开放服务器产品目录、用户实名、订单、实例和工单相关数据库契约；实名供应商集成只覆盖支付宝/微信侧实名核验，不开放支付交易。实例集成只覆盖 MCP PVE client API 已提供能力，不开放通用 PVE 运维管理。
...
- 订单状态包含 `pending`、`provisioning`、`fulfilled`、`cancelled`、`closed`，不引入支付状态。
...
- 订单不创建支付单、不扣减库存。管理员触发交付后，服务端基于实例交付映射调用 MCP PVE client API 创建 VM。
