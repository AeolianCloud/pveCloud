...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API，并开放最小 Web 公开站点配置和用户登录会话接口。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户登录会话接口和健康检查

当前契约不再把 Worker、产品、订单、支付、实例、工单和其他用户端业务流纳入现阶段交付范围。
...
当前仓库除站点配置和用户登录会话外，不再把其它 `/api/*` 作为现行后端契约。
...
用户端公开 API 边界，当前只承载 `GET /api/site-config`。
...
当前不再保留用户端 API、产品、订单、支付、实例、工单、异步任务或用户端账号等业务域数据库契约。
