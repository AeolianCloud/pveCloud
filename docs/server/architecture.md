...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API、用户端业务 API 和后台 Worker，并开放最小 Web 公开站点配置、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单、支付、钱包、发票、实例、异步任务和工单能力。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单接口、支付接口、钱包接口、发票接口、实例接口、工单接口、管理端异步任务接口和健康检查
- Worker：领取并执行通用异步任务，负责实例 operation 同步、实例到期提醒、到期释放和通知发送

当前契约重新开放用户账号自助、用户实名、服务器产品目录、订单、支付、钱包、发票、续费订单、实例、通用异步任务、通知和工单。支付一期只开放中国大陆商户的微信 Native 扫码、微信 H5、支付宝电脑网页支付、支付宝手机网页支付和钱包余额支付；发票 v1 只开放已支付订单的人工电子普通发票申请和管理端运营处理。JSAPI/openid、小程序支付、专票、红冲、第三方开票平台和部分退款仍不纳入当前阶段交付范围。实例交付和到期释放通过后端内部 MCP PVE client API 调用上游 PVE 适配服务，不向前端暴露 PVE 节点、存储、磁盘来源或 VMID。
...
- Web 发票 API：`/api/invoices/*`、`/api/invoice-eligible-orders`
- Web 实例 API：`/api/instances/*`
- Web 工单 API：`/api/tickets/*`

当前仓库除站点配置、用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、支付、钱包、发票、实例和工单外，不再把其它 `/api/*` 作为现行后端契约。管理端异步任务只通过 `/admin-api/async-tasks/*` 暴露，不对用户端开放。
...
- `usecase/admin/invoice`：发票申请列表、详情、受理、驳回、开票登记、后台备注和 PDF 访问
...
- `usecase/web/invoice`：当前用户可开票订单、发票申请、列表、详情、取消和 PDF 下载
...
当前后端领域边界以管理端基础能力为主，同时已重新开放用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、支付、钱包、发票、续费订单、实例、异步任务、通知和工单：
...
用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单、支付、钱包、发票、续费订单、实例、通用异步任务、通知和工单已经重新纳入当前阶段契约。部分退款、JSAPI/openid、小程序支付和其它用户端业务流仍从当前阶段契约中收口。实例只覆盖 MCP PVE client API 当前具备的创建、查询、启动、停止、删除和异步操作查询能力。
...
当前重新开放服务器产品目录、用户实名、订单、支付、钱包、发票、实例、工单、通用异步任务和实例生命周期相关数据库契约；实名供应商集成只覆盖支付宝/微信侧实名核验，支付供应商集成只覆盖微信支付、支付宝支付的一期网页/扫码交易能力和钱包余额支付，发票 v1 不接第三方开票平台。实例集成只覆盖 MCP PVE client API 已提供能力，不开放通用 PVE 运维管理。
...
- 退款前必须检查关联订单是否存在 `pending`、`processing` 或 `issued` 发票申请；存在有效发票申请时拒绝退款。v1 不支持红冲或开票后在线退款。
...
- 用户端发票只挂载 `/api/invoices/*` 和 `/api/invoice-eligible-orders`；管理端发票只挂载 `/admin-api/invoices/*`，并通过独立发票运营菜单开放。
- 发票 v1 只支持已支付订单的人工电子普通发票申请，允许当前用户把多笔可开票订单合并为一张申请。
- 发票申请状态包含 `pending`、`processing`、`issued`、`rejected` 和 `cancelled`。
- `pending`、`processing` 和 `issued` 申请占用订单开票资格；`rejected` 和 `cancelled` 释放占用。
- 用户创建发票申请必须同事务锁定订单、校验归属、支付状态、退款状态、币种和有效占用，再写入申请和订单明细。
- 用户创建幂等依赖 `user_id + client_token`；重复提交返回已有申请，不重复占用订单。
- 管理端可受理 `pending` 申请、驳回 `pending` 或 `processing` 申请、对 `processing` 申请登记开票并绑定 PDF 文件。
- 发票 PDF 复用文件附件存储，但必须通过发票下载接口按用户归属或管理端权限裁决，不开放公网匿名访问。
- 受理、驳回、开票登记和后台备注更新必须写普通后台操作审计。
- v1 不支持增值税专用发票、红冲、作废、第三方开票平台、发票邮件发送、钱包充值开票、部分退款或开票后在线退款。
...
- 工单可选关联当前用户自己的订单和实例，用于售后沟通和实例排障定位；同时传入订单与实例时，实例必须来源于同一订单。工单不承载支付处理、PVE 节点、资源池、库存扣减、实例操作或自动交付能力。
...
- 通用 PVE 运维管理
- 真实短信供应商集成
- JSAPI/openid、小程序支付、专票、红冲、第三方开票平台、部分退款、提现、人工调账、余额转账和自动对账批处理
