...
│  │
│  ├─ openapi/
│  │  └─ spec.go                         # OpenAPI 3.x API 规范文件加载、校验和输出
│  │
│  ├─ api/
│  │  ├─ web/                            # 官网 + 用户端接口
...
第一期下单明确为一单一实例，不支持一个订单批量购买多个实例。镜像必须按用户选择的地域过滤，只允许选择该地域和目标 PVE 节点已配置模板映射的镜像。
...
前期使用独立 `cmd/worker` 进程 + 数据库任务表实现轻量任务队列。任务表结构以 `docs/server/database/design.md` 和 `server/migrations/001_init.sql` 中的 `async_tasks` 为准，本节只定义任务行为。
...
配置通过 YAML 文件加载，`config.example.yaml` 保留必要示例，真实配置文件使用 `config.yaml` 且不提交仓库。API 和 worker 进程都支持 `-config config.yaml` 指定配置路径。
...
运行中不做配置局部热更新。配置、代码或 OpenAPI 文档变更后，本地开发通过 Air 热重载重启进程，生产环境通过进程管理器或发布流程重启。
