...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API，并开放最小 Web 公开站点配置、用户登录会话接口和服务器产品目录展示接口。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户登录会话接口、服务器产品目录展示接口和健康检查

当前契约重新开放服务器产品目录；订单、支付、实例、Worker、工单和其他用户端业务流仍不纳入现阶段交付范围。
...
当前仓库除站点配置、用户登录会话和服务器产品目录外，不再把其它 `/api/*` 作为现行后端契约。
...
用户端公开 API 边界，当前承载 `GET /api/site-config`、用户登录会话接口和 `GET /api/server-catalog`。
...
服务器产品目录已经重新纳入当前阶段契约，用于后台维护和 Web 公开展示。订单、支付、实例、工单、异步任务和其它用户端业务流仍从当前阶段契约中收口。后续如需恢复，必须先更新文档与迁移。
