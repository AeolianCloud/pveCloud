...
`pveCloud` 后端当前以 Go 基础后台为主，现行契约范围聚焦管理端 API，并开放最小 Web 公开站点配置、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单 MVP 和工单 MVP。

## 进程职责

- API：提供管理端 HTTP 接口、Web 公开站点配置接口、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录展示接口、订单 MVP 接口、工单 MVP 接口和健康检查

当前契约重新开放用户账号自助、用户实名、服务器产品目录、订单 MVP 和工单 MVP；支付、实例、Worker 和其他用户端业务流仍不纳入现阶段交付范围。这里的支付宝/微信仅用于实名核验，不代表支付能力已开放。
...
当前仓库除站点配置、用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单 MVP 和工单 MVP 外，不再把其它 `/api/*` 作为现行后端契约。
...
当前后端领域边界以管理端基础能力为主，同时已重新开放用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单 MVP 和工单 MVP：
...
用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单 MVP 和工单 MVP 已经重新纳入当前阶段契约。支付、实例、异步任务和其它用户端业务流仍从当前阶段契约中收口。后续如需恢复，必须先更新文档与迁移。
...
当前重新开放服务器产品目录、用户实名、订单和工单相关数据库契约；实名供应商集成只覆盖支付宝/微信侧实名核验，不开放支付交易。支付、实例、异步任务等业务域仍不属于当前阶段。
...
- 订单状态只包含 `pending`、`cancelled`、`closed`，不提前引入支付或交付状态。
...
- 订单不创建支付单、不创建实例、不绑定 PVE 节点、不扣减库存、不承诺自动交付。
- 管理端备注、取消和关闭订单必须写入普通后台操作审计。
...
- 工单回复和附件引用必须同事务写入。
- 管理端回复和关闭工单必须写入普通后台操作审计。
- 工单不承载客服指派、SLA、支付、实例、PVE 节点、资源池、库存扣减或自动交付能力。
