...
- 用户端工单接口属于受保护接口，必须基于当前登录用户做工单、订单关联和附件资源归属校验，不得信任请求体或路径中的用户 ID、订单归属、工单归属、附件归属或状态
...
- 用户端业务 API（公开站点配置、用户账号自助、用户实名、服务器产品目录、订单、支付、实例和工单接口除外）
- 钱包
- 发票、JSAPI/openid、小程序支付、部分退款和自动对账批处理
- 通用 PVE 运维管理
