...
订单保存用户购买意向和后台处理所需快照。新购订单由管理员触发交付；续费订单用于延长已有实例服务期。当前阶段不接真实支付网关，但保留支付状态和交易字段，后续支付回调必须复用续费确认逻辑。
...
- 列表项包含订单编号、订单类型、支付状态、状态、产品名称、套餐名称、计费周期、网络类型、关联实例编号、订单金额和创建/取消/关闭时间；订单状态允许 `pending`、`provisioning`、`fulfilled`、`cancelled`、`closed`
...
- 约束：
  ...
  - 当前阶段 `payment_status` 初始为 `unpaid`，真实支付网关未开放
...
- 约束：
  - 仅 `failed` 任务可重试
  - 重试时必须清空锁定字段，状态回到 `pending`，并递增人工重试记录
  - 必须写入后台操作审计
