...
当前用户账号自助、支付宝/微信侧个人实名、真实支付一期和实例交付阶段开放以下外部集成契约：
...
发票、JSAPI/openid、小程序支付、部分退款、提现、人工调账、余额转账、自动对账批处理和其它外部业务集成仍不属于现阶段文档承诺范围。钱包 v1 只复用现有支付宝/微信支付适配做充值，不新增外部钱包供应商。MCP PVE client API 不作为 pveCloud 对外接口暴露，用户端和管理端只能通过订单、支付、钱包、实例和实例操作业务接口间接使用。
