...
`*_test.go` 是否保留，不默认由 AI 决定。新增测试文件只有在维护者明确要求、任务验收要求，或对应 owner docs 明确要求长期回归时才应保留。
如果测试只是为当前修改提供验证支撑而未被明确要求保留，验证后默认清理。
临时探测脚本、一次性样例、调试数据生成器和未确认保留的测试文件，不要当作长期资产留在仓库里。
