...
- service：支付创建幂等、配置不完整拒绝创建、重复回调、金额不一致、订单非法状态、续费成功写生效记录、新购成功投递自动交付、钱包余额支付扣款、充值入账、余额支付退款退回钱包、退款渠道失败不回滚服务期。
- handler：用户只能查询自己的支付和钱包；支付回调不要求 Bearer Token 但验签失败必须拒绝；管理端低权限不能退款、同步、重试自动交付或查看钱包。
- repository/migration：支付交易唯一键、退款唯一键、支付生效记录唯一键、钱包账户唯一键、钱包流水幂等键、钱包充值幂等键、微信平台公钥 ID 配置项和订单 `error`/`refunded` 状态兼容。
