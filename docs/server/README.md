...
- 后端当前只把 `/admin-api/*`、公开站点配置接口、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录接口、用户端订单 MVP 接口和 `/healthz` 作为现行契约
- `cmd/api` 负责当前唯一的服务入口
- MariaDB 是基础后台事实来源
- Redis 是运行时基础依赖，用于管理端会话、验证码、短 TTL 状态、限流和缓存

当前除公开站点配置、用户账号自助接口、支付宝/微信侧用户实名接口、服务器产品目录接口和用户端订单 MVP 外，不再把用户端支付、实例、工单和异步任务视为现行业务契约。
