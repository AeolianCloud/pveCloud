...
支付成功处理必须在事务内完成订单、支付、余额流水或任务创建等关键变更。重复回调不能重复入账、重复创建实例或重复修改订单。
...
如果 PVE 创建失败，任务进入 `failed`，订单进入 `failed` 或保持 `provisioning` 并等待补偿策略。后台应提供人工重试、取消、备注能力。
...
订单超时任务必须跳过已支付、已取消、已关闭的订单。关闭第三方支付单失败时应记录日志，并允许补偿任务重试。
...
前期可以使用 goroutine + 数据库任务表实现轻量任务队列。数据库表结构后续单独设计，这里只定义任务行为。
