...
- API：HTTP 接口
- `admin`：管理端前端
- `web`：用户端前端；当前承载公开站点、产品目录、用户账号自助、支付宝/微信侧个人实名入口、订单 MVP 和工单 MVP，不代表支付或实例 API 已开放
...
说明：`web/` 已存在。当前用户端开放公开站点配置、用户账号自助、支付宝/微信侧用户实名、服务器产品目录、订单 MVP 和工单 MVP 接口，不代表支付或实例 API 已开放。
