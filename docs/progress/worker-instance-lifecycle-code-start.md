...
- 接口：`docs/server/api/README.md`、`docs/server/api/instances-tasks.md`、`docs/server/api/orders-payments-wallet.md`
...
6. `docs/server/api/README.md`
7. `docs/server/api/instances-tasks.md`
8. `docs/server/api/orders-payments-wallet.md`
9. `docs/server/database/design.md`
10. `server/migrations/037_worker_instance_lifecycle.sql`
11. `server/config.example.yaml`
12. `docs/admin/pages/async-tasks.md`
13. `docs/admin/pages/instance-management.md`
14. `docs/admin/pages/order-management.md`
15. `docs/web/pages/instances.md`
16. `docs/web/pages/orders.md`
17. `docs/web/pages/order-detail.md`
...
- 用户可为自己的未释放实例创建续费订单。
- 订单列表和详情展示 `purchase`、`renewal` 两类订单，以及支付摘要状态；真实支付入口以后续支付 owner docs 为准。
...
- 续费确认的事务：订单状态、支付摘要状态、`paid_at`、实例 `expires_at` 和审计必须一致。
...
- 短信：只占位，不调用真实供应商。
- 前端展示：支付入口以后续支付 owner docs 为准，不出现 MCP 未提供的运维入口。
