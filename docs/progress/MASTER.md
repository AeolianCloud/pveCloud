...
当前数据库契约已重新开放用户端账号会话、用户实名、服务器产品目录、订单、真实支付一期、钱包 v1、发票 v1、续费订单、实例交付、通用异步任务、通知和工单；专票、红冲、第三方开票平台等发票增强能力仍不在当前阶段。
操作日志查询已按系统设置子页面口径开放；审计日志内部写入能力仍保留。

## 当前推进阶段

MCP PVE 实例交付阶段已进入实现复盘状态；实例生命周期、续费订单和 Worker 异步任务已进入实现收尾状态；真实支付闭环一期和钱包 v1 已完成代码提交，待真实渠道端到端验收与生产告警配置确认。发票 v1 已完成文档、迁移、后端、管理端和用户端实现，当前只开放已支付订单的人工电子普通发票申请和运营登记开票。

当前已开放管理端交付映射、MCP 只读资源、订单触发交付、实例列表/详情/开机/关机/释放/同步；已开放用户端实例列表、详情、启动和停止。真实支付闭环一期已开放用户端支付发起/查询/回调、管理端支付管理、退款、支付成功自动交付和续费支付生效记录。钱包 v1 已开放用户充值、余额支付、余额支付退款退回钱包和管理端只读钱包管理。发票 v1 已开放用户端发票列表、申请、详情、取消和 PDF 下载，管理端发票运营列表、详情、受理、驳回、开票登记、后台备注和 PDF 下载。工单关联实例排障入口已开放用户端工单关联当前用户自己的业务实例编号、管理端工单按实例编号筛选和从工单跳转实例管理定位排障；不开放新增 PVE 运维能力。
...
2026-05-25 发票 v1 完成与稳定记录：已补齐 `docs/server/invoices.md`、`docs/server/api/endpoints.md`、`docs/server/database/design.md`、`docs/admin/pages/invoice-management.md`、`docs/web/pages/invoices.md`、`server/migrations/041_invoice_v1.sql` 及相关安全、审计、前后端架构入口；后端执行 `PVECLOUD_TEST_MYSQL_DSN=... GOCACHE=/tmp/pvecloud-go-build go test ./internal/delivery/http/web/invoice ./internal/delivery/http/admin/invoice -v` 和 `PVECLOUD_TEST_MYSQL_DSN=... GOCACHE=/tmp/pvecloud-go-build go test ./...` 通过；`admin` 与 `web` 均执行 `bash -ic 'bun run build'` 通过。当前稳定范围为人工电子普通发票 v1：用户端可开票订单筛选、申请、幂等、取消、列表、详情和 PDF 下载；管理端列表、详情、受理、驳回、开票登记、后台备注和 PDF 下载；退款前有效发票拦截、文件引用校验、权限鉴权、错误码映射和后台审计均已有回归覆盖。该记录不代表专票、红冲、作废、第三方开票平台、钱包充值开票、部分退款、开票后在线退款或邮件发送能力开放。

2026-05-25 Dashboard 业务运营看板完成记录：已更新 `docs/admin/pages/dashboard.md` 和 `docs/server/api/endpoints.md`，`GET /admin-api/dashboard` 在保留基础后台指标的同时新增 `business_metrics` 只读业务指标；首批覆盖待处理订单、交付异常订单、异常实例、失败异步任务、待后台处理工单、待处理发票和支付异常。后端已将 Dashboard 聚合收敛为固定白名单查询，不接收前端动态表名或 where 条件；管理端 Dashboard 新增“业务待办与异常”卡片，按目标页面权限控制跳转入口。验证记录：`PVECLOUD_TEST_MYSQL_DSN=... GOCACHE=/tmp/pvecloud-go-build go test ./internal/usecase/admin/dashboard -v` 通过，`PVECLOUD_TEST_MYSQL_DSN=... GOCACHE=/tmp/pvecloud-go-build go test ./...` 通过，`admin` 执行 `bash -ic 'bun run build'` 通过。该能力只做只读汇总和入口跳转，不直接执行订单处理、退款、实例操作、任务重试、工单回复或发票流转，也不开放专票、红冲、部分退款、自动对账、实例控制台、重装、快照或备份等未开放能力。
...
后续若要开放 JSAPI/openid、小程序支付、专票、红冲、第三方开票平台、部分退款、提现、人工调账、余额转账、自动对账批处理、实例更多 MCP 运维能力等真实用户端业务，必须先补齐并确认：

- 相关用户端 `/api/*` 和管理端 `/admin-api/*` 接口契约
- 新增能力所需数据库契约、迁移和配置示例
- 支付、退款、钱包、发票增强或实例新增能力的业务流程说明
- 对应管理端和用户端页面范围
- 用户端路由、权限和请求包装口径
