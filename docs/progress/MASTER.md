...
当前数据库契约已重新开放用户端账号会话、用户实名、服务器产品目录、订单 MVP 和工单 MVP；支付、实例和异步任务等业务域仍不在当前阶段。
...
当前已开放用户端工单列表、创建、详情、回复、关闭和附件访问；已开放管理端工单列表、详情、回复、关闭和附件访问。
...
- 再次开放支付、实例、异步任务或其它管理端页面前，必须先完成文档确认
