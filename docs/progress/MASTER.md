...
MCP PVE 实例交付阶段已进入实现复盘状态；实例生命周期、续费订单和 Worker 异步任务进入文档契约确认阶段。

当前已开放管理端交付映射、MCP 只读资源、订单触发交付、实例列表/详情/开机/关机/释放/同步；已开放用户端实例列表、详情、启动和停止。下一阶段契约计划新增用户端续费订单、管理端续费确认、实例服务期、到期提醒、到期后可配置自动释放和管理端异步任务页面。
