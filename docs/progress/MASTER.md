...
当前数据库契约已重新开放用户端账号会话、用户实名、服务器产品目录和订单 MVP；支付、实例、异步任务和工单等业务域仍不在当前阶段。
操作日志查询已按系统设置子页面口径开放；审计日志内部写入能力仍保留。

## 当前推进阶段

当前 Web 用户端前端页面准备清空重做。

本阶段目标：删除现有 `web/` 页面实现、业务路由和 Vue 应用入口，仅保留请求边界骨架，后续重新设计用户端页面。
...
- 再次开放支付、实例、工单、异步任务或其它管理端页面前，必须先完成文档确认
...
- 历史 Web 页面曾读取公开产品目录并展示产品、套餐、价格、销售地域、服务器系统模板、简介和状态；当前 Web 页面已进入清空重做，不再展示产品目录
- 订单 MVP 只表示购买意向和后台人工处理入口；Web 不展示支付、实例、PVE 节点、资源池、库存扣减或自动开通能力
...
后续若要开放支付、实例、工单等真实用户端业务，必须先补齐并确认：
