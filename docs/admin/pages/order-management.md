...
本页面不支持后台创建订单，不发起退款，不直接同步支付渠道，不包含库存扣减或通用 PVE 管理能力。实例交付通过实例管理域能力触发，只能基于已有用户端订单；续费订单由用户端创建，后台可人工确认并延长实例服务期。真实支付和退款处理入口在支付管理页。
...
- 页面可展示最近支付/退款摘要和支付管理跳转入口，但不得展示商户密钥、完整回调 payload、完整上游响应、PVE 节点、库存扣减或用户侧自动交付承诺。
