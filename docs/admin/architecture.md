...
- 订单管理以独立一级菜单开放，用于查看和处理用户端订单；不支持后台创建订单，订单详情只展示支付摘要和跳转入口。
- 支付管理以独立一级菜单开放，用于查看支付流水、退款流水、支付详情、发起全额退款、同步渠道状态和重试真实支付后的自动交付失败订单。
- 钱包管理以独立一级菜单开放，用于只读查看用户钱包、余额、充值记录和钱包流水；不支持管理端人工加款、扣款、冻结、提现或余额转账。
- 发票运营以独立一级菜单开放，用于查看用户端发票申请、受理、驳回、登记人工开票结果和维护后台备注；不接第三方开票平台，不创建用户订单或用户发票申请。
- 实例管理以独立一级菜单开放，用于维护交付映射、查看实例、从订单触发交付、同步 MCP 状态和执行开机、关机、释放；不开放通用 PVE 运维管理。
- 异步任务以独立一级菜单开放，用于查看 Worker 后台任务并重试失败任务；不开放用户端任务查询。
- 工单管理以独立一级菜单开放，用于查看和处理用户端工单；支持后台内部指派、转派、协作者、内部备注、内部 SLA 时限、标签、优先级升级和关联实例排障入口；不支持后台创建用户工单，不承载支付、实例开通或 PVE 操作。
