...
本阶段开放用户注册、密码找回、登录、登录态恢复、自动刷新、退出、订单 MVP 和工单 MVP，不开放支付或实例流程。

登录页是官网的一部分，包含官网导航菜单和页脚。
...
- 用户端登录页仅在开发模式显示“记住账号密码”按钮或勾选项，生产构建不得显示该入口，也不得读取或写入该开发辅助凭据。
- 开发模式下用户开启“记住账号密码”后，登录成功才将当前登录账号和密码保存到用户端本地浏览器存储；关闭该选项或清空登录表单时应清理已保存的开发辅助凭据。
- 该能力只用于本地开发提效，不参与登录态恢复、token refresh、redirect、订单、工单、实名或任何服务端流程。
