...
- 用户可以为可支付订单选择支付宝、微信或钱包余额支付；余额不足等失败原因以后端返回为准。
- 用户可以从已支付且可开票订单进入发票申请流程；若已有有效发票申请，展示发票摘要和详情入口。
- 页面展示新购订单和续费订单。
...
- 关联实例编号和续费确认时间
- 最近支付摘要
- 支付入口信息、钱包余额支付入口和支付页跳转入口
- 发票申请入口、发票摘要或发票详情跳转入口

页面不得展示后台备注、商户密钥、完整回调 payload、发票后台备注、PDF 物理路径、PVE 节点、资源池、库存扣减或上游自动开通进度。若订单已交付或关联实例，可展示跳转到实例详情的业务入口。
...
- `GET /api/invoice-eligible-orders` - 当前用户可开票订单列表
- `POST /api/invoices` - 当前用户创建发票申请
- `GET /api/invoices/{invoice_no}` - 当前用户发票详情
