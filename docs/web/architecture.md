...
- Web 用户端前端已重新开放官网展示、用户账号自助、用户实名、产品目录、订单 MVP 和工单 MVP。
- 当前不开放支付页、实例管理页、钱包页、发票、SSH 密钥管理、PVE 节点、资源池、库存扣减或自动开通流程。
- 后续重新开放支付、实例、钱包或其它用户端业务页面前必须先更新本文档和对应页面契约。
...
- 用户端仅展示 owner docs 已开放的订单 MVP 和工单能力；不得展示尚未开放的支付、实例、资源交付或用户侧 SLA 承诺。

## 当前阶段范围

当前阶段开放官网展示、用户账号自助页面、订单 MVP 和工单 MVP，不开放支付、实例等业务流程。
...
- CTA 可以进入订单确认或创建流程；不得承诺支付成功、自动开通、实例交付或用户侧 SLA。
...
- 不展示客服指派、内部 SLA、内部备注、协作者、支付状态、实例状态、PVE 节点、资源池、库存扣减或自动开通进度。
...
- 用户端展示页不能隐式承诺后台尚未开放的支付、实例、PVE 或自动交付能力；订单只按 MVP 契约展示购买意向和后台人工处理入口，工单只按 MVP 契约展示沟通和附件能力

## 后续阶段要求

后续开放支付、实例等真实用户端业务前，至少需要补齐：
