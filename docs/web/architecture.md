...
Web 前端面向官网、公区产品展示和用户中心。接口边界是 `/api/*`，最终接口契约以 `docs/server/api/` 和对应后端业务文档为准。
...
用户端页面只能依赖 `docs/server/api/` 中 `/api/*` 契约和对应业务文档。订单金额、可购买套餐、镜像、地域和支付状态必须以后端返回为准，前端不做最终业务裁决。
