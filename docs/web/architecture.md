...
- 当前进入服务器产品目录阶段，开放公开服务器产品目录读取能力；订单、支付、实例或工单等业务 API 仍不开放。
...
这些页面承载信息架构、服务器产品目录展示、用户登录态和未来接入点，不承载注册、下单、支付、实例或工单流程。
...
- 本阶段不新增站点配置、用户登录会话和服务器产品目录以外的用户端业务接口契约。
...
- 当前阶段不自动调用 `POST /api/auth/refresh`；token 过期、token 无效或会话失效时回到登录流程。
- 本阶段不持久化订单、实例或工单状态。
...
- `web/` 不调用 `/admin-api/*`
- 本阶段只新增公开站点配置、用户登录会话和服务器产品目录接口，不新增订单、支付、实例或工单接口
