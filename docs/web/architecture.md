...
- Web 用户端前端已重新开放官网展示、用户账号自助、用户实名、产品目录、订单、实例和工单。
- 当前不开放支付页、钱包页、发票、SSH 密钥管理、PVE 节点、资源池、库存扣减或通用 PVE 运维流程。
- 后续重新开放支付、钱包或其它用户端业务页面前必须先更新本文档和对应页面契约。
...
- 用户端仅展示 owner docs 已开放的订单、实例和工单能力；不得展示尚未开放的支付、钱包、资源池、通用 PVE 运维或用户侧 SLA 承诺。

## 当前阶段范围

当前阶段开放官网展示、用户账号自助页面、订单、实例和工单，不开放支付等业务流程。
...
- 不展示支付状态、PVE 节点、资源池、库存扣减或上游自动开通进度。
...
- 用户端展示页不能隐式承诺后台尚未开放的支付、通用 PVE 或自动交付能力；订单按契约展示购买意向和后台处理入口，实例按契约展示已交付云主机和基础电源操作，工单按契约展示沟通和附件能力
