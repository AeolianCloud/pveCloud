...
- 写 owner docs 时，状态变更、外部副作用、异步任务、资源分配、通知、回调或其它跨系统业务流程必须先按 `references/contract-quality.md` 明确状态机、锁粒度、幂等键、失败恢复、审计和测试口径。
- 不把长耗时外部调用放进长事务。
...
- 新增或修改后端代码时，必须同步补充注释；函数、方法、结构体、字段、常量、变量、条件分支、错误处理、字段映射、DTO 赋值和常规 CRUD 都要覆盖。
- service 中的多步骤业务编排应在关键步骤前写行前或块级注释，说明该步骤保护的业务约束；状态机、事务边界、幂等键、锁粒度、外部副作用、失败恢复、权限裁决和敏感信息处理必须写清业务意图。
- repository 中涉及动态筛选、锁、唯一性、软删除、分页上限、批量查询或 SQL 安全白名单时，应写注释说明安全和一致性边界；普通查询、字段扫描和分页也要用简短注释说明用途。
- integration 中涉及第三方协议、超时、错误映射、异步 operation 或上游响应摘要时，应写注释说明为何这样收敛；简单请求组装和响应转换也要有对应注释。
- 简单字段映射、普通 DTO 赋值、显而易见的条件返回和常规 CRUD 不再作为免注释场景；可以用简短注释覆盖作用，必要时逐行注释。
