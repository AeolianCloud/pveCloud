...
const handleUpdateProfile = async () => {
  ...
  try {
    ...
    authStore.applyAuthState(data)
    syncFormFromUser()
    success.value = '资料更新成功'
  } catch (err) {
    error.value = getApiErrorMessage(err, '更新失败，请稍后重试')
  } finally {
    loading.value = false
  }
}
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    error.value = getApiErrorMessage(err, '密码修改失败，请检查当前密码是否正确')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="page-reveal bg-white">
    <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div class="stagger-reveal mt-8 grid gap-6 lg:grid-cols-2">
        <section class="interactive-card rounded-[1.5rem] border border-neutral-950 bg-white p-6 shadow-[8px_8px_0_#111]">
          ...
          <form class="mt-6 space-y-5" @submit.prevent="handleUpdateProfile">
            ...
            <div>
              ...
              <input id="email" v-model="email" type="email" class="field-focus w-full rounded-xl border border-neutral-300 px-4 py-3 text-sm outline-none focus:border-neutral-950" />
            </div>
            <div>
              <label for="display-name" class="mb-2 block text-sm font-black text-neutral-800">显示名称</label>
              <input id="display-name" v-model="displayName" type="text" class="field-focus w-full rounded-xl border border-neutral-300 px-4 py-3 text-sm outline-none focus:border-neutral-950" placeholder="请输入显示名称" />
            </div>
        ...
        <section class="interactive-card rounded-[1.5rem] border border-neutral-200 bg-neutral-50 p-6">
          ...
          <form class="mt-6 space-y-5" @submit.prevent="handleChangePassword">
            <div>
              ...
              <input id="current-password" v-model="currentPassword" type="password" class="field-focus w-full rounded-xl border border-neutral-300 px-4 py-3 text-sm outline-none focus:border-neutral-950" placeholder="请输入当前密码" />
            </div>
            <div>
              <label for="new-password" class="mb-2 block text-sm font-black text-neutral-800">新密码</label>
              <input id="new-password" v-model="newPassword" type="password" class="field-focus w-full rounded-xl border border-neutral-300 px-4 py-3 text-sm outline-none focus:border-neutral-950" placeholder="请输入新密码" />
            </div>
            <div>
              <label for="confirm-new-password" class="mb-2 block text-sm font-black text-neutral-800">确认新密码</label>
              <input id="confirm-new-password" v-model="confirmPassword" type="password" class="field-focus w-full rounded-xl border border-neutral-300 px-4 py-3 text-sm outline-none focus:border-neutral-950" placeholder="请再次输入新密码" />
            </div>
