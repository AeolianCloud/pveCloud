    ...
    <div class="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <article v-else-if="ticket" class="rounded-[1.5rem] border border-neutral-200 bg-white p-5 shadow-[8px_8px_0_#111] sm:p-6">
        <div class="grid gap-4 border-b border-neutral-200 pb-5 md:grid-cols-[minmax(0,1fr)_9rem] md:items-start">
          <div class="min-w-0"><p class="truncate text-xs font-black uppercase tracking-[0.16em] text-neutral-500">{{ ticket.ticket_no }}</p><h1 class="mt-2 text-2xl font-black text-neutral-950">{{ ticket.title }}</h1><p class="mt-2 text-sm text-neutral-500">{{ categoryText[ticket.category] }} · {{ priorityText[ticket.priority] }} · {{ ticket.order_no || '未关联订单' }}</p></div>
