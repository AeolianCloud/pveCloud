      ...
      <div v-else-if="orders.length" class="space-y-4">
        <article v-for="order in orders" :key="order.order_no" class="interactive-card rounded-[1.5rem] border border-neutral-200 bg-white p-6">
          <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div><div class="text-xs font-black uppercase tracking-[0.16em] text-neutral-500">{{ order.order_no }}</div><h2 class="mt-2 text-xl font-black text-neutral-950">{{ order.product_name }} · {{ order.plan_name }}</h2><p class="mt-2 text-sm text-neutral-500">{{ cycleText[order.billing_cycle] || order.billing_cycle }} · {{ order.network_type_name }} · {{ order.created_at }}</p></div>
