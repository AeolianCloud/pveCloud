...
<template>
  <div class="page-reveal bg-white">
    <div class="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <button type="button" class="mb-6 text-sm font-black text-neutral-600 underline" @click="router.back()">返回</button>
      ...
      <article v-else-if="order" class="rounded-[2rem] border border-neutral-200 bg-white p-8 shadow-[10px_10px_0_#111]">
        <div class="flex flex-col justify-between gap-4 border-b border-neutral-200 pb-6 md:flex-row md:items-start"><div><p class="text-xs font-black uppercase tracking-[0.16em] text-neutral-500">{{ order.order_no }}</p><h1 class="mt-3 text-3xl font-black text-neutral-950">{{ order.product_name }} · {{ order.plan_name }}</h1><p class="mt-3 text-sm text-neutral-500">订单只表示购买意向，不代表支付或实例交付。</p></div><div class="text-left md:text-right"><div class="text-3xl font-black">{{ formatMoney(order.total_amount_cents) }}</div><span class="mt-2 inline-flex rounded-full border px-3 py-1 text-xs font-black">{{ statusText[order.status] }}</span></div></div>
        ...
        <section v-if="order.user_note" class="mt-8"><h2 class="text-lg font-black">用户备注</h2><p class="mt-2 rounded-2xl bg-neutral-50 p-4 text-sm text-neutral-600">{{ order.user_note }}</p></section>
        <div class="mt-8 flex flex-wrap gap-3"><button v-if="order.status === 'pending'" type="button" class="rounded-full border border-red-300 px-5 py-2 text-sm font-black text-red-700 hover:bg-red-50" @click="cancel">取消订单</button><RouterLink to="/user/orders" class="rounded-full border border-neutral-950 px-5 py-2 text-sm font-black hover:bg-neutral-950 hover:text-white">返回订单列表</RouterLink></div>
      </article>
    </div>
  </div>
</template>
